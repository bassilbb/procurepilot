<x-app-layout>
    <x-slot name="header">Edit Procurement Request</x-slot>

    <x-page-header :title="$request->reference" description="Update the request details" />

    <form method="POST" action="{{ route('requests.update', $request) }}" x-data="requestForm({{ $request->items->toJson() }})">
        @csrf
        @method('PUT')
        ...
        <div class="hd-card p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $request->title)" required />
                </div>
                <div class="md:col-span-2">
                    <x-input-label for="justification" :value="__('Justification')" />
                    <textarea id="justification" name="justification" rows="3" class="mt-1 w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 shadow-sm">{{ old('justification', $request->justification) }}</textarea>
                </div>
                <div>
                    ...
                    <select id="department_id" name="department_id" class="mt-1 w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-emerald-500">
                        ...
                        @foreach ($departments as $dept)
                            <option value="{{ $dept->id }}" @selected(old('department_id', $request->department_id) == $dept->id)>{{ $dept->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="budget_code" class="block mt-1 w-full" type="text" name="budget_code" :value="old('budget_code', $request->budget_code)" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="required_date" class="block mt-1 w-full" type="date" name="required_date" :value="old('required_date', $request->required_date?->format('Y-m-d'))" />
                </div>
                <div>
                    <x-input-label for="currency" :value="__('Currency')" />
                    <x-text-input id="currency" class="block mt-1 w-full" type="text" name="currency" :value="old('currency', $request->currency)" />
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <button type="submit" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg">Update Request</button>
        </div>
    </form>

    <script>
        function requestForm(items) {
            return {
                items: items.length ? items.map(i => ({ description: i.description, quantity: i.quantity, unit: i.unit, estimated_unit_cost: i.estimated_unit_cost })) : [{ description: '', quantity: 1, unit: '', estimated_unit_cost: 0 }],
                addItem() {
